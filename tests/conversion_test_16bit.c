...
int main(int argc, char **argv) {
	...
	printf("Matrix Conversion to 16-bit Test");
#ifndef SERIAL
	printf(" (NEON)");
#endif
#ifdef DEBUG
	...
	buffer16bit = (int16_t*)malloc(inw*inh*2);
	if(!buffer16bit){
		printf("Error: failed to allocate memory for 16-bit buffer.\n\n");
		ret = 50; goto exit;
	}

	// We don't have a 32bit -> 16bit routine so we'll just hastily convert the numbers ourselves
	float ftemp;
	for(size_t i = 0; i < inw*inh; i++) {
		ftemp = input0.d[i] * quant_mul_factor[int_bits];

        // Perform type conversion and clamp if required
        if(ftemp < -32768) // out of negative bounds
            buffer16bit[i] = -32768;
        else if(ftemp < 32767) // Withing range; no clamp
            buffer16bit[i] = (int16_t)ftemp;
	...
	*/
	...
	for(size_t iter = 0; iter < iterations; iter++) {
		startClock();
		// Convert back
		matrixFrom16bit(buffer16bit, int_bits, &input0);
...
exit:
	...
	if(buffer16bit) { free(buffer16bit); }
	return ret;
}
